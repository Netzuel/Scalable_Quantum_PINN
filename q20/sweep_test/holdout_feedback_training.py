...
def main() -> None:
    ...
    parser.add_argument("--base-agp-terms", type=int, default=1024)
    parser.add_argument("--rounds", type=int, default=1)
    parser.add_argument("--add-residual-terms", type=int, default=1024)
    parser.add_argument("--epochs-per-round", type=int, default=1000)
    ...
    parser.add_argument("--output-root", type=Path, default=RUN_DIR / "runs" / "holdout_feedback")
    ...
    settings = settings_for_support(payload, args.base_agp_terms)
    settings = replace(
        settings,
        epochs=int(args.epochs_per_round),
        lr=float(args.lr) if args.lr is not None else settings.lr,
        optimizer=str(args.optimizer) if args.optimizer is not None else settings.optimizer,
        intermediate_top_k=intermediate_top_k,
        device=args.device if args.device is not None else settings.device,
    )
    base_run = RUN_DIR / "runs" / f"agp_{args.base_agp_terms}"
    ...
    support_sizes = (
        [int(value) for value in sweep_support.get("agp_terms", [576, 768, 1024, 1536, 2048])]
    ...
    output_dir = output_root / f"agp_{args.base_agp_terms}_add_{args.add_residual_terms}_rounds_{args.rounds}"
    data_dir = output_dir / "Models_Data"
    data_dir.mkdir(parents=True, exist_ok=True)
    ...
    print(f"evaluate_feedback_baseline agp_terms={args.base_agp_terms}")
    ...
    baseline_row["run_dir"] = str(base_run.relative_to(output_dir)) if False else str(base_run)
    ...
    for round_index in range(1, args.rounds + 1):
        additions = select_residual_additions(
            ...
            add_terms=int(args.add_residual_terms),
        ...
        print(
            ...
            f"residual_terms={len(residual_labels)} added={len(additions)} epochs={settings.epochs}"
        )
    ...
    summary_path = output_dir / "Models_Data" / f"holdout_feedback_summary_residual_{args.residual_top_k}.json"
    try:
        ...
        json.dumps(
            {
                ...
                "base_agp_terms": args.base_agp_terms,
                "agp_fraction_of_full_basis": f"{Decimal(args.base_agp_terms) / full_basis:.12E}",
                "rounds": round_rows,
            },
            indent=2,
        )
